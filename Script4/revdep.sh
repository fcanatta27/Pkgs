#!/bin/sh
#  Copyright (c) 2025 by fcanatta  (fcanatta64@gmail.com)
#
...
rebuild() {	
	for allpkg in $(pkg deplist $ALLPKG | awk '{print $2}'); do
...
check_pythonmodules() {
	...
	for i in /usr/lib/python3.*; do
		...
		brokenpkg="$brokenpkg $(pkg provide $i/$ | awk '{print $1}')"
	done
}
...
command -v pkg >/dev/null 2>&1 || {
	echo "'pkg' not found in \$PATH!"
	exit 1
}

# package database directory
PKGDB_DIR="$(pkg --print-dbdir)"
